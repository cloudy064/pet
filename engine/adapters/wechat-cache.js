...
const INDEX = 'pipi_engine_assets_v1',
  DIRECTORY = 'pipi-engine-v1';
const CHECK_MS = 10 * 60 * 1000,
  MAX_BYTES = 48 * 1024 * 1024;
const instances = new WeakMap();
const validManifest = (data) =>
  data &&
  data.version === 1 &&
  data.assets &&
  Object.keys(data.assets).length > 0 &&
  Object.values(data.assets).every(
    (a) =>
      a &&
      a.anchor &&
      a.crop &&
      ...
      Array.isArray(a.frameMap) &&
      Array.isArray(a.durations) &&
      Array.isArray(a.restFrames) &&
      a.frameMap.length > 0 &&
      a.frameMap.length === a.durations.length &&
      a.frameMap.every((i) => a.tiles[i]) &&
      a.durations.every((t) => Number.isFinite(t) && t > 0) &&
...
function createPetAssetCache(wxApi, options = {}) {
  const now = options.now || Date.now,
    limit = options.maxBytes || MAX_BYTES;
  ...
  let state;
  try {
    state = wxApi.getStorageSync(INDEX);
  } catch (_) {}
  ...
  const save = () => {
    try {
      wxApi.setStorageSync(INDEX, state);
      return true;
    } catch (_) {
      return false;
    }
  ...
  async function manifest(base) {
    const old = state.manifests[base],
      cached = old && validManifest(old.data) && old.data;
    ...
    if (!checks.has(base)) {
      ...
      const job = call(wxApi, 'request', { url: base + '/manifest.json', timeout: 15000, header })
        .then((r) => {
          const data =
            r.statusCode === 304 && cached
              ? cached
              : typeof r.data === 'string'
                ? JSON.parse(r.data)
                : r.data;
          if ((r.statusCode !== 200 && r.statusCode !== 304) || !validManifest(data))
            throw Error('Invalid pet manifest');
          const headers = Object.fromEntries(
            Object.entries(r.header || {}).map(([k, v]) => [k.toLowerCase(), v])
          );
          if (token === generation) {
            state.manifests[base] = {
              data,
              checked: now(),
  ...
  async function acquire(base, page, id, token) {
    const md5 = (page.md5 || '').toLowerCase(),
      legacyId = base + '/' + page.file;
    ...
    if (fs && old && owned(old.path)) {
      try {
        ...
        if (
          info.digest.toLowerCase() === (md5 || old.md5) &&
          info.size === old.bytes &&
          (!page.bytes || info.size === page.bytes)
        ) {
          old.used = now();
          if (!state.files[id]) {
            state.files[id] = old;
            delete state.files[legacyId];
          }
          save();
          return { path: old.path, hit: true };
        }
      } catch (_) {}
      await serialized(() => remove(id));
    }
    const response = await call(wxApi, 'downloadFile', {
      url: page.url || base + '/' + page.file + (md5 ? '?v=' + md5 : ''),
      timeout: 20000,
    });
    ...
    try {
      ...
      if ((md5 && info.digest.toLowerCase() !== md5) || (page.bytes && info.size !== page.bytes))
        throw Error('Pet atlas checksum mismatch');
      return await serialized(async () => {
        if (token !== generation || info.size > limit) return { path: temp, temp };
        const path =
          root +
          '/' +
          info.digest.toLowerCase() +
          '-' +
          now().toString(16) +
          '-' +
  ...
  async function loadImage(base, page, decode) {
    ...
    const id = page.md5 ? 'md5:' + page.md5.toLowerCase() : page.url || base + '/' + page.file;
    let job = downloads.get(id);
    if (!job) {
      job = { users: 0, promise: acquire(base, page, id, generation) };
    ...
    try {
      ...
      return await decode(source.path);
    } catch (error) {
      if (source && !error.petDisposed)
        await serialized(() => {
          return remove(id).then(save);
        });
      throw error;
    } finally {
      if (--job.users === 0) {
  ...
  async function clear() {
    ...
    await serialized(async () => {
      ...
      state = { version: 1, files: {}, manifests: {} };
      retryAt.clear();
      save();
    });
  }
