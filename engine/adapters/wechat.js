...
const call = (object, name, args) =>
  new Promise((resolve, reject) => object[name]({ ...args, success: resolve, fail: reject }));

class WechatAdapter {
  constructor(canvas, wxApi) {
    if (!canvas || typeof canvas.createImage !== 'function')
      throw new TypeError('WeChat Canvas 2D node is required');
    if (!wxApi) throw new TypeError('Pass the wx API explicitly');
    this.canvas = canvas;
    this.wx = wxApi;
    this.cache = getPetAssetCache(wxApi);
  ...
  createCanvas(width, height) {
    return this.wx.createOffscreenCanvas({ type: '2d', width, height });
  }
  ...
  fetchManifest(baseURL) {
    return this.cache.manifest(baseURL);
  }
  decode(src) {
    return new Promise((resolve, reject) => {
      ...
      image.onerror = () => {
        image.onload = image.onerror = null;
        reject(new Error('WeChat image decode failed'));
      };
      image.src = src;
    });
  }
    ...
    if (!this.embedded.has(id)) {
      const task = (async () => {
        const fs = this.wx.getFileSystemManager(),
          root = this.wx.env.USER_DATA_PATH + '/pipi-engine-starter';
        const path = root + '/' + id + '.png';
        const info = await call(fs, 'getFileInfo', { filePath: path, digestAlgorithm: 'md5' }).catch(
          () => null
        );
        if (!info || info.digest.toLowerCase() !== id || info.size !== page.bytes) {
          ...
          if (written.digest.toLowerCase() !== id || written.size !== page.bytes)
            throw new Error('Embedded atlas checksum mismatch');
        }
        return path;
      })();
      this.embedded.set(id, task);
      task.catch(() => this.embedded.delete(id));
    }
  ...
  loadImage(page, baseURL) {
    ...
    const plain = page.url.split(/[?#]/)[0],
      slash = plain.lastIndexOf('/');
    const url =
      page.url.split('#')[0] + (page.md5 ? (page.url.includes('?') ? '&' : '?') + 'v=' + page.md5 : '');
    return this.cache.loadImage(
      plain.slice(0, slash),
      { ...page, url, file: plain.slice(slash + 1) },
      (src) => this.decode(src)
    );
  }
  releaseImage(image) {
  ...
  createAudio(source, callbacks) {
    ...
    audio.onError(() => callbacks.error(new Error('WeChat audio playback failed')));
    return {
      play: () => audio.play(),
      pause: () => audio.pause(),
      stop: () => audio.stop(),
      dispose: () => audio.destroy(),
    };
  }
  bind() {
    return () => {};
