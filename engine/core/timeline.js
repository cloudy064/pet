...
const range = (first, last) =>
  Array.from({ length: Math.abs(last - first) + 1 }, (_, i) => first + i * Math.sign(last - first));
...
class AnimationPlan {
  ...
  sample() {
    throw new Error('AnimationPlan.sample must be implemented');
  }
  release() {}
}
...
class Timeline extends AnimationPlan {
  constructor(segments, assets) {
    super();
    if (!segments.length) throw new TypeError('Timeline requires segments');
    this.segments = segments.map((input, index) => {
      ...
      if (!frames.length || frames.some((f) => !Number.isInteger(f) || f < 0 || f >= asset.frameMap.length))
        throw new RangeError('Frame outside asset: ' + input.asset);
      ...
      const repeat =
        input.repeat === Infinity
          ? Infinity
          : input.repeat === undefined
            ? 1
            : positive(input.repeat, 'Repeat count');
      if (repeat === Infinity && index !== segments.length - 1)
        throw new TypeError('Only the last segment may loop forever');
      const speed = input.speed === undefined ? 1 : positive(input.speed, 'Segment speed');
      const times = durations.map((t) => t / speed),
        cycle = times.reduce((a, b) => a + b, 0);
  ...
  sample(elapsed) {
    let time = clamp(elapsed, 0, this.duration),
    ...
    return {
      layers: [{ asset: segment.asset, frame }],
      segment,
      segmentTime: time,
      frame,
      asset: segment.asset,
...
class SustainPlan extends AnimationPlan {
  constructor({ open, loop, close, holdMs = 0, sustain = false }, assets) {
    super();
    this.open = new Timeline([open], assets);
    this.loop = new Timeline([loop], assets);
    this.close = new Timeline([close], assets);
    this.assetIds = [...new Set([...this.open.assetIds, ...this.loop.assetIds, ...this.close.assetIds])];
    this.closeAt = sustain
      ? Infinity
      : this.open.duration + Math.max(1, Math.ceil(holdMs / this.loop.duration)) * this.loop.duration;
  ...
  release(elapsed) {
    ...
    this.closeAt = this.open.duration + cycles * this.loop.duration;
    this.duration = this.closeAt + this.close.duration;
  }
  sample(elapsed) {
    if (elapsed < this.open.duration) return { ...this.open.sample(elapsed), phase: 'open' };
    if (elapsed < this.closeAt)
      return { ...this.loop.sample((elapsed - this.open.duration) % this.loop.duration), phase: 'loop' };
...
class CombinedPlan extends AnimationPlan {
  constructor(plans) {
    super();
    if (plans.some((p) => !Number.isFinite(p.duration)))
      throw new TypeError('Sequences cannot contain an unbounded loop');
    this.plans = plans;
    this.duration = plans.reduce((n, p) => n + p.duration, 0);
    this.assetIds = [...new Set(plans.flatMap((p) => p.assetIds))];
  }
  sample(elapsed) {
    let time = Math.max(0, elapsed);
    for (let i = 0; i < this.plans.length; i++) {
      const plan = this.plans[i];
      if (time < plan.duration || i === this.plans.length - 1) return plan.sample(time);
      time -= plan.duration;
    }
  }
}
